...
func (m *mysqlFisherRepository) Fetch(ctx context.Context) (res []domain.Fisher, err error) {
	query := `SELECT f.id, f.nik, f.name, f.address, f.ship_type, f.abk_total, f.status, f.phone_number, f.created_at, f.updated_at, u.name
			FROM fisher AS f
...
func (m *mysqlFisherRepository) Update(ctx context.Context, f *domain.Fisher) (err error) {
	query := `UPDATE fisher SET user_id=?, nik=?, name=?, address=?, ship_type=?, abk_total=?, status=?, phone_number=?, created_at=?, updated_at=? WHERE id = ?`
	...
	res, err := stmt.ExecContext(ctx, f.UserID, f.Nik, f.Name, f.Address, f.ShipType, f.AbkTotal, f.Status, f.PhoneNumber, f.CreatedAt, f.UpdatedAt, f.ID)
	if err != nil {
		return
	}
...
func (m *mysqlFisherRepository) Store(ctx context.Context, f *domain.Fisher) (err error) {
	query := `INSERT fisher SET user_id=?, nik=?, name=?, address=?, ship_type=?, abk_total=?, status=?, phone_number=?, created_at=?, updated_at=?`
	stmt, err := m.Conn.PrepareContext(ctx, query)
	if err != nil {
		return
	}

	res, err := stmt.ExecContext(ctx, f.UserID, f.Nik, f.Name, f.Address, f.ShipType, f.AbkTotal, f.Status, f.PhoneNumber, f.CreatedAt, f.UpdatedAt)
	if err != nil {
		return
	}
